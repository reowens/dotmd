...
- **`runlist decisions --json` says what each decision blocks.** Each row carries `blocks: [{ doc, line, kind, section, text }]`: what its record says it blocks or gates (`stated`), and every unticked checklist item (`item`) or frontmatter blocker (`blocker`) that names its id, alone or in a written range such as `A2 to A12`; in another document only where the id sits beside a link to the decision's document. Rows also carry `question`, read even from an incomplete record, and `docTitle`. The text list is unchanged.
- **`runlist show <file...> [--json]`** prints one document's card, read-only: title, status, summary, current state, next step, blockers, checklist counts, the plans and docs its frontmatter names (each with title and status, a missing one marked) and the other documents its body links to.
...
- **`runlist decisions` lets a heading with a dated ruling govern the items under it** (`Decisions, ruled 2025-01-01`), as a shouted heading already did. A bare lowercase `open` in a heading still does not.
...
- **`runlist flag sync <check-name> [@findings.json | -]`** lets a check outside runlist feed the list: it takes everything the check reports now, as a JSON array of `{ file, line?, text, severity? }`, adds the new findings and resolves the check's flags it no longer reports. A finding is matched to its flag on file and text, so a problem reported at a new line after an edit stays one flag.
...
- **`runlist new decision <plan> --question "…" @record.md`** adds a numbered entry to a plan's top-level decisions section: a `Disposition: OPEN.` line (or `HELD` with `--disposition held`) and the record as its body, with `## Decisions` created before `## Version History` when the plan has none. A decisions heading nested inside a workstream is left alone. The record and the question are required, and `updated:` is bumped in the same write. Ids are read only where a decision item starts (a heading, a list or bold lead, a table row, a register row), never from prose, where the same shape is as often a job code or a citation of another plan's decision.
  With `export const decisions = { register: { file, statusLine } }`, ids are one sequence across the corpus: the new id follows the highest the register or the plan uses, and the register block (the fenced block whose first line carries `statusLine`) gets the entry's row, the question plus the required `--answers`, linking the record. Both files are written in one locked set that refuses if either changed after it was read. The heading and prefix are `section` and `prefix` in the same block.
