...
test('buildSyncPlan identifies stale global installations', () => {
  const plan = buildSyncPlan([
    { dotmdPath: '/a/dotmd', version: '0.69.0', runlistVersion: '0.69.0', rlVersion: '0.69.0', npmPath: '/a/npm', prefix: '/a' },
    { dotmdPath: '/b/dotmd', version: '0.50.2', runlistVersion: null, rlVersion: null, npmPath: '/b/npm', prefix: '/b' },
    { dotmdPath: '/c/dotmd', version: null, runlistVersion: null, rlVersion: null, npmPath: null },
  ], '0.69.0');
...
test('syncGlobalCliCopies updates each stale PATH-visible prefix and verifies again', () => {
  ...
  let inspection = 0;
  const before = [
    { dotmdPath: '/homebrew/bin/dotmd', runlistPath: '/homebrew/bin/runlist', rlPath: '/homebrew/bin/rl', version: '0.69.0', runlistVersion: '0.69.0', rlVersion: '0.69.0', npmPath: '/homebrew/bin/npm', prefix: '/homebrew' },
    { dotmdPath: '/nvm/bin/dotmd', runlistPath: null, rlPath: null, version: '0.68.0', runlistVersion: null, rlVersion: null, npmPath: '/nvm/bin/npm', prefix: '/nvm' },
  ];
  const after = before.map(entry => ({
    ...entry,
    runlistPath: entry.dotmdPath.replace(/dotmd$/, 'runlist'),
    rlPath: entry.dotmdPath.replace(/dotmd$/, 'rl'),
    version: '0.69.0',
    runlistVersion: '0.69.0',
    rlVersion: '0.69.0',
  }));
...
test('syncGlobalCliCopies refuses an unrepairable stale binary', () => {
  assert.throws(() => syncGlobalCliCopies('0.69.0', {
    inspect: () => [{ dotmdPath: '/orphan/dotmd', version: '0.50.2', runlistVersion: null, rlVersion: null, npmPath: null }],
    write: () => {},
  }), /no sibling npm/);
});
...
test('syncGlobalCliCopies never overwrites but fails on a stale non-global shim', () => {
  const installed = [];
  const entries = [
    { dotmdPath: '/homebrew/bin/dotmd', version: '0.69.0', runlistVersion: '0.69.0', rlVersion: '0.69.0', npmPath: '/homebrew/bin/npm', prefix: '/homebrew', managed: true },
    { dotmdPath: '/volta/bin/dotmd', version: '0.50.0', runlistVersion: null, rlVersion: null, npmPath: '/volta/bin/npm', prefix: '/volta/tools/node', managed: false },
  ];
  const output = [];
  assert.throws(() => syncGlobalCliCopies('0.69.0', {
    inspect: () => entries,
...
test('syncGlobalCliCopies repairs a current bridge install that only lacks rl', () => {
  let inspection = 0;
  const installed = [];
  const before = [{
    dotmdPath: '/homebrew/bin/dotmd', version: '0.77.1',
    runlistPath: '/homebrew/bin/runlist', runlistVersion: '0.77.1',
    rlPath: null, rlVersion: null,
    npmPath: '/homebrew/bin/npm', prefix: '/homebrew', managed: true,
  }];
  const after = [{
    ...before[0], rlPath: '/homebrew/bin/rl', rlVersion: '0.77.1',
  }];
  syncGlobalCliCopies('0.77.1', {
    inspect: () => inspection++ === 0 ? before : after,
    install: entry => { installed.push(entry.dotmdPath); return { status: 0 }; },
    write: () => {},
  });
  assert.deepEqual(installed, ['/homebrew/bin/dotmd']);
});
