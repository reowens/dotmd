...
// recognizable as generated forever, or an old install turns into a "foreign"
// file that install/update/remove refuse to touch.
...
function banner(version) {
  return [
    ...
    '// Generated by `runlist install opencode`. Refreshed by `runlist update`.',
    '// Hand edits are overwritten — remove with `runlist install opencode --remove`.',
    '',
  ].join('\n');
}
...
// The banner a generated file carries — { marker, version } — or null when the
// file is absent or carries neither banner. A file without a banner is the
// user's own and is never written or removed — same rule the retired
// slash-command scaffolding followed. The earliest banner in the file wins.
export function readGeneratedBanner(filePath) {
  let contents;
  ...
  for (const marker of GENERATED_MARKERS) {
    ...
    if (at >= 0 && (found === null || at < found.at)) found = { marker, at };
  }
  if (!found) return null;
  const rest = contents.slice(found.at + found.marker.length);
  return { marker: found.marker, version: rest.split('\n', 1)[0].trim() || null };
}

export function installedVersion(filePath) {
  return readGeneratedBanner(filePath)?.version ?? null;
}
...
export function describeSessionIdentity(opts = {}) {
  ...
  if (source.scope === 'process' && source.host === 'OpenCode') {
    advice.push(opencode.exists
      ...
      : 'runlist install opencode  — for a per-session identity');
...
export function degradedIdentityNotice(repoRoot, opts = {}) {
  ...
  const lines = installed
    ? ['[runlist] This OpenCode session started before the runlist integration loaded, so its plan',
      '        ownership is still process-scoped. Restart OpenCode to pick it up.']
    : ['[runlist] OpenCode detected, runlist integration not installed — plan ownership is scoped to',
      '        the OpenCode process, so another session in it can release plans this one claimed,',
      '        and no session-start briefing runs. Fix once: runlist install opencode'];
...
export function planClaudeInstall({ installed, hasClaude, remove = false } = {}) {
  ...
  if (installed) return [{ kind: 'skip', reason: `runlist plugin already installed (${installed.version ?? 'unknown version'})` }];
