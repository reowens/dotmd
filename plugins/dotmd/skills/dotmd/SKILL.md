...
- **A plan claimed by a session that is gone** blocks `set`, `archive` and `baton` on it. `runlist doctor --claims` lists every claim with whether its session is live; `runlist doctor --claims --apply` releases the ones whose session is provably gone and sets the plan back to `active`. A claim runlist cannot judge (no recorded process, another machine) is released only with `--older-than <24h|3d>`, which is the person's call. SessionStart names dead claims when there are any.
